...
#define ADE9000_ZX_LP_SEL 0x0000    /*Line period and zero crossing obtained from combined signals VA,VB and VC*/ 
...
#define ADE9000_VLEVEL  0x00603EE8    /*Assuming Vnom=1/2 of full scale. 
...
#define ADE9000_EP_CFG 0xE013       /*Enable energy accumulation, accumulate samples at 8ksps*/
                    /*latch energy accumulation after EGYRDY*/
                    /*If accumulation is changed to half line cycle mode, change EGY_TIME*/
#define ADE9000_EGY_TIME 0x0257//0x04AF//0077         /*Accumulate 8000 samples*/
//0077=1seg
//0x04AF=10seg
...
struct ResampledWfbDataNoncoh
{
  int32_t VA_Resampled[WFB_ELEMENT_ARRAY_SIZE];
  int32_t IA_Resampled[WFB_ELEMENT_ARRAY_SIZE];
  int32_t VB_Resampled[WFB_ELEMENT_ARRAY_SIZE];
  int32_t IB_Resampled[WFB_ELEMENT_ARRAY_SIZE];
  int32_t VC_Resampled[WFB_ELEMENT_ARRAY_SIZE];
  int32_t IC_Resampled[WFB_ELEMENT_ARRAY_SIZE];
  int32_t IN_Resampled[WFB_ELEMENT_ARRAY_SIZE];
};
...
struct WatthrRegValue
{
  int32_t WatthrValue_A;
  int32_t WatthrValue_B;
  int32_t WatthrValue_C;
  float wattHrA;
  float wattHrB;
  float wattHrC;
};
struct FundWatthrRegValue
{
  int32_t FundWatthrValue_A;
  int32_t FundWatthrValue_B;
  int32_t FundWatthrValue_C;
  float FundwattHrA;
  float FundwattHrB;
  float FundwattHrC;
};

struct VARhrRegValue
{
  int32_t VARhrValue_A;
  int32_t VARhrValue_B;
  int32_t VARhrValue_C;
  float VARHrA;
  float VARHrB;
  float VARHrC;
};
struct FundVARhrRegValue
{
  int32_t FundVARhrValue_A;
  int32_t FundVARhrValue_B;
  int32_t FundVARhrValue_C;
  float FundVARHrA;
  float FundVARHrB;
  float FundVARHrC;
};
...
class ADE9000Class
{
  public:
    ...
    void SPI_Burst_Read_Resampled_Noncoh_Wfb(uint16_t Address, uint16_t Read_Element_Length, ResampledWfbDataNoncoh *ResampledData);
    void SPI_Burst_Read_Resampled_Coh_Wfb(uint16_t Address, uint16_t Read_Element_Length, ResampledWfbDataCoh *ResampledData);
    ...
    void ReadWatthrRegsnValue(WatthrRegValue *Data);
    void ReadVARhrRegsnValue(VARhrRegValue *Data);
    void ReadFundWatthrRegsnValue(FundWatthrRegValue *Data);
    void ReadFundVARhrRegsnValue(FundVARhrRegValue *Data);
    void ReadPfValue(PfRegValue *Data);
    /*OMAR FUNCTTIONS*/
    float vrmsConstantA = 0.00001347659422;
    float irmsConstantA = 0.000003752321953;
    float wattConstantA = 0.00676523548;
    
    float vrmsConstantB = 0.00001356608396;
    float irmsConstantB = 0.000003743844137;
    float wattConstantB = 0.006756054929;    
    
    float vrmsConstantC = 0.00001365881025;
    float irmsConstantC = 0.000003725298496;
    float wattConstantC = 0.006749345875;    
    float wattHrConstant = 0.000001903109;
